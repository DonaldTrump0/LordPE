...
// 插入一行到进程列表窗口
void addToProcessListView(HWND hListProcess, wchar_t* processName, DWORD processID, DWORD imageBase, DWORD sizeOfImage)
{
	LV_ITEM item;
	wchar_t str[20];
...
// 插入一行到模块列表窗口
void addToModuleListView(HWND hListModule, wchar_t* moduleName, DWORD moduleAddr)
{
	LVITEM item;
	memset(&item, 0, sizeof(LV_ITEM));
	item.mask = LVIF_TEXT;
...
{
	DWORD processIDs[1024], cbNeeded, processNum;
...
// 遍历进程的所有模块
void enumModule(HWND hModuleList, HWND hListProcess)
{
	// 获取选择行
	DWORD row = SendMessage(hListProcess, LVM_GETNEXTITEM, -1, LVNI_SELECTED);
	if (row == -1)
	{
		return;
	}
	...
	for (size_t i = 0; i < cbNeeded / sizeof(DWORD); i++)
	{
		...
		// 将模块信息插入到模块列表窗口中
		addToModuleListView(hModuleList, moduleName, (DWORD)hModule[i]);
	}
}
